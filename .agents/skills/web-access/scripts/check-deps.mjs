...
async function httpGetJson(url, timeoutMs = 3000) {
  try {
    ...
    try {
      return { data: JSON.parse(await res.text()), errorCode: null, errorMessage: null };
    } catch {
      return { data: null, errorCode: 'INVALID_RESPONSE', errorMessage: '响应不是有效 JSON' };
    }
  } catch (error) {
    return {
      data: null,
      errorCode: error.cause?.code || error.code || error.name || 'UNKNOWN',
      errorMessage: error.cause?.message || error.message || String(error),
    };
  }
}
...
function printLoopbackPermissionHint(port, errorCode) {
  console.log(`proxy: 当前执行环境无法访问本机端口 127.0.0.1:${port} (${errorCode})`);
  console.log('  Agent 处理顺序：');
  console.log('    1. 不要重复启动浏览器或 Proxy；通过平台正常的本机端口访问授权重新运行 node check-deps.mjs');
  console.log('    2. 不得绕过沙箱；若授权环境仍失败，再根据保留的错误码继续诊断');
}
...
async function ensureProxy(expectedBrowserId, browserOverride) {
  ...
  const healthResult = await httpGetJson(healthUrl);
  if (isPermissionError(healthResult.errorCode)) {
    printLoopbackPermissionHint(PROXY_PORT, healthResult.errorCode);
    return false;
  }
  ...
  for (let i = 1; i <= 15; i++) {
    ...
    if (Array.isArray(result.data)) {
      const newHealth = (await httpGetJson(healthUrl)).data;
    ...
    if (isPermissionError(result.errorCode)) {
      printLoopbackPermissionHint(PROXY_PORT, result.errorCode);
      return false;
    }
    if (i === 1 && result.errorCode !== 'ECONNREFUSED') {
      console.log('⚠️  浏览器可能有授权弹窗，请点击「允许」后等待连接...');
    }
    await new Promise((r) => setTimeout(r, 1000));
  }

  console.log(`❌ Proxy 连接超时${lastFailure?.errorCode ? ` (${lastFailure.errorCode})` : ''}`);
  console.log('  请先根据错误码判断 Proxy、本机端口权限或浏览器授权状态，不要机械重复启动浏览器');
...
async function resolveAndReport(override) {
  ...
  switch (result.kind) {
    ...
    case 'unreachable': {
      const browsers = result.browser ? [result.browser] : result.unreachable;
      console.log('browser: unreachable — 找到有效的调试端口记录，但当前进程无法访问');
      for (const browser of browsers) {
        console.log(`  ${browser.label} (${browser.id}): 127.0.0.1:${browser.port} (${browser.errorCode})`);
      }
      const permissionDenied = browsers.some(browser => isPermissionError(browser.errorCode));
      console.log('  Agent 处理顺序：');
      if (permissionDenied) {
        console.log('    1. 当前执行环境阻止了本机回环端口访问；通过平台正常的本机端口访问授权重新运行 node check-deps.mjs');
        console.log('    2. 不要重复打开浏览器，不得绕过沙箱');
      } else {
        console.log('    1. 先检查当前执行环境的本机端口权限，并在正常授权环境中重跑一次；不要机械重复运行或反复打开浏览器');
        console.log('    2. 只有授权环境仍确认该端口无监听时，才重新打开对应浏览器并检查远程调试开关');
      }
      return { proceed: false, exitCode: 1 };
    }
