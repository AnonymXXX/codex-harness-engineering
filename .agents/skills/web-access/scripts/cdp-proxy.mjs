...
async function discoverChromePort() {
  ...
  if (result.kind === 'unreachable') {
    ...
    const details = browsers
      .map(browser => `${browser.label} 127.0.0.1:${browser.port} (${browser.errorCode})`)
      .join('、');
    throw new Error(
      `找到浏览器调试端口记录，但当前进程无法访问：${details}。` +
      '先通过平台正常的本机端口访问授权重试；不要重复启动浏览器，也不得绕过沙箱。'
    );
  }
