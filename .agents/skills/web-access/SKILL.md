...
在限制网络访问的执行环境中，`check-deps.mjs` 以及后续所有对 `127.0.0.1:3456` 的请求都需要使用平台正常的本机端口访问授权：

- 遇到 `EPERM` / `EACCES` 时，不得把它解释为 Chrome 未启动或远程调试未开启，也不得通过其他工具绕过沙箱
- 遇到 `ECONNREFUSED` 时，先在正常授权环境中复核一次；只有仍确认端口无监听，才重新打开浏览器或检查远程调试开关
- 一次失败后先读取并判断脚本保留的错误码，不要机械重复运行 `check-deps.mjs`、反复执行 `open -a` 或重复启动 Proxy
