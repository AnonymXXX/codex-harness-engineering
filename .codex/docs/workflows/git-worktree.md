...
- Bootstrap grants no authority to run migrations, seeds, database resets, deploys, uploads, production service calls, or other external mutations. Check existing authorization for the exact operation, target, data, and side effects; execute only when covered. If new authorization is required, ask before that operation while continuing independent safe work.
- If bootstrap fails, diagnose the environment or dependency issue and pause only edits or checks that require the missing dependency or generated artifact. Continue independent safe edits and non-dependent checks. Do not treat missing dependencies as application failures or report blocked validation as passed; complete required validation before integration.
...
- browser acceptance state and verification evidence, plus a separate integration basis and operation/target authorization source, following [Browser Acceptance](harness-engineering.md#browser-acceptance);
...
- Treat validated commit, eligible local auto-merge, separately authorized remote integration, and cleanup as the normal autonomous completion path. Do not end with "ready to merge" or ask the user to repeat a merge request when the applicable gates pass. Record acceptance evidence and integration basis separately under [Browser Acceptance](harness-engineering.md#browser-acceptance). A commit-only request does not itself authorize integration.
...
For UI-affecting work, an explicit current-task push request, including push-only or commit-and-push wording, permits continuing without unrequested browser acceptance after project-required validation and integration preflight. Do not ask separately whether acceptance was completed. Follow [Browser Acceptance](harness-engineering.md#browser-acceptance) to record the actual evidence and the request as integration basis; do not turn the request into a passing test result. This does not override a required project approval, expressly required acceptance, or a known unresolved failure. The push request does not authorize production operations, tags, force pushes, other remotes, or MR/PR creation or merge; their existing authorization rules still apply.
...
For qualifying work, record integration basis as `high-confidence-policy` and save the automatic-check results, static final-diff review, and target branch as evidence. Record actual browser acceptance separately under [Browser Acceptance](harness-engineering.md#browser-acceptance). This policy does not override required project approval, expressly required acceptance, or known unresolved failures. Then complete validation, commit, preflight, normal non-forced push when needed, remote ancestry verification, applicable post-integration validation, and task worktree/branch cleanup. A failed check, doubtful review, skipped critical check, dirty or unisolated worktree, ineligible or prohibited target, excluded side effect, `STOP`, or `MR_REQUIRED` stops auto-integration and retains the task worktree. `MR_REQUIRED` is report-only unless the user separately authorizes the MR/PR flow.
...
- An explicit MR/PR request supplies authorization only for the requested operations. Apply the same unrequested-browser-acceptance rule as an explicit push, while keeping acceptance evidence separate under [Browser Acceptance](harness-engineering.md#browser-acceptance). A push-only request does not authorize MR/PR creation or merge.
